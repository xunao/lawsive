...
<title>律氏中文网-调研页</title>
<meta name="keywords" content="律氏-调研" />
<meta name="description" content="律氏-调研" />
<?php	
		include ('../../frame.php');
		use_jquery_ui();
		...
		$db=get_db();
		$news_id = $_GET['id'];
		$record=$db->query("select * from lawsive.article where id='{$news_id}' and resource_type='research'");
...
<!--       			<div id="l_t4">字号 　　　　　背景 </div><div id="l_t5">收藏 电邮 打印 评论<a href=""><font color="#9E2F2F">[1条]</font></a>　&nbsp;<a href=""><font color="#9E2F2F">英文</font></a>　&nbsp;<a href=""><font color="#9E2F2F">对照</font></a></div>-->
          		...
          		<div id="content">
          		    <div><?php echo $record[0]->content;?></div>
          		    	<div class="con_m"><a href="<?php echo $record[0]->research_src;?>" target="_blank">现在就参与>></a></div>
          		    <?php if (count($latest_news)==1) {?>
          		    <div class="cont_b1"><?php echo $record[0]->author;?>上一篇文章：</div>
          			<div class="cont_b2"><a href="/news/research&report/research.php?id=<?php echo $latest_news[0]->id;?>"><?php echo $latest_news[0]->title;?></a> <font class="cont_b3"><?php echo date('Y-m-d',strtotime($latest_news[0]->created_at));?></font></div>
          			<?php }?>
          		</div>
