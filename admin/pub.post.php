<?php
  include_once('../frame.php');
	if('del_total'== $_POST['post_type']){
		...
		if($_POST['tbname'] == 'eb_rich'){
			$db->execute("delete from eb_rich_company");
			$db->execute("delete from eb_rich_fortune");
		}
		if($_POST['tbname'] == 'eb_company'){
			$db->execute("delete from eb_rich_company");
		}
		exit();
	}
  	if('del_all'== $_POST['post_type']){
		...
		$db->execute("delete from {$_POST['tbname']} where id in ({$_POST['ids']})");
  		if($_POST['tbname'] == 'eb_rich'){
			$db->execute("delete from eb_rich_company where rich_id in  ({$_POST['ids']})");
			$db->execute("delete from eb_rich_fortune where rich_id in  ({$_POST['ids']})");
		}
		exit();
	}
	...
	if("del"==$_POST['post_type'])
	{
		if($_POST['db_table'] == $tb_news){
				...
				if($post->language_tag==1){
					$db->execute("delete from eb_news_relationship where english_news_id={$_POST['del_id']}");
				}else{
					$db->query("select english_news_id from eb_news_relationship where chinese_news_id={$_POST['del_id']}");
					if($db->move_first()){
						$e_id = $db->field_by_index(0);
						$db->execute("delete from eb_news_relationship where english_news_id={$e_id}");
						$post->delete($e_id);
					}
				}
			...
			}elseif($_POST['db_table'] == 'admin_menu'){
				$db=get_db();
				$right = new Table('rights');
				$right->find('first',array('conditions' => "name='{$_POST['del_id']}' and type=2"));
				if($right->id){
					$db->execute("delete from role_rights where rights_id = {$right->id}");
					$right->delete();
				}
			...
			if(isset($_POST['rela'])){
				$db = get_db();
				$db->execute("delete from eb_position_relation where type='{$_POST['rela']}' and news_id={$_POST['del_id']}");
			}
