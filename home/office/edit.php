...
<?php	
		...
		include ('../../frame.php');
		use_jquery_ui();
		css_include_tag('person_public','diary','home/office');
		js_include_tag('login','admin/office/office');
		use_ckeditor();
		$user = member::current();
		if(!$user)
		{
...
<body>
	<div id="ibody">
      	...
      	<div id="diary_box">
      		<div id="diary_title">
      			<img src="../../../images/diary/logo_diary.jpg" />律师&律所推荐
      			<div id="e_ret"><a href="/home/office/">&gt;&gt;返回我的律师&律所推荐首页</a></div>	
      		</div>
      		<div id="d_m">
	      		<form method="post" enctype="multipart/form-data" action="edit.post.php">
	      			...
	      			<div class="al">照片路径：<input id="upfile" type="file" name="post[photo]" value="<?php echo $article->photo;?>" /></div>
...
<!--	      				<div id="ed_post"></div>-->
	      				<div id="ed_reco">
		      				选择推荐分类
				      		    <select style="width:80px;" name="post[resource_type]" id="of_reco">
									<option value="lawyer">律师推荐</option>
									<option value="lawfirm">律所推荐</option>
								</select>
						</div>
	      				<div id="ed_nr">
	      					<button type="submit" id="dia_edit" class="ed_sub">提交</button>
...
<!--	      					<input type="hidden" id="article_id" name="article_id" value="<?php echo $article_id?>">-->
	      					<input type="hidden" id="category_id" name="post[category]" value="<?php echo $article->category;?>">
	      					<input type="hidden" name="post[member_id]"  id="member_id" value="<?php echo $user->id;?>">
	      					<input type="hidden" name="post[member_name]" id="member_name"  value="<?php echo $user->name;?>">
	      				</div>
